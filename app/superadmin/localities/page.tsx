...
export default function SuperAdminLocalitiesPage() {
  ...
  useEffect(() => {
    adminApi.getLocalities()
      .then(res => setLocalities(res?.localities || []))
      .catch(console.error)
      .finally(() => setLoading(false))
  }, [])

  const saveToBackend = async (newLocalities: any[]) => {
    try {
      await adminApi.saveLocalities(newLocalities)
    } catch(e: any) {
  ...
  const toggleLocalityStatus = async (id: string) => {
    ...
    <PageTransition>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <Card className="lg:col-span-1 bg-white/60 dark:bg-slate-900/50 backdrop-blur-2xl border-white dark:border-slate-800 shadow-xl rounded-3xl h-fit">
            ...
            <CardContent>
              <form onSubmit={handleAddLocality} className="space-y-5">
                ...
                <Button 
                  ...
                  className="w-full h-12 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-base shadow-lg shadow-indigo-200 dark:shadow-none"
                >
                  <Plus className="mr-2 h-5 w-5" /> Add Locality
                </Button>
              </form>
            </CardContent>
          </Card>
          ...
          {/* Localities List */}
          <Card className="lg:col-span-2 bg-white/60 dark:bg-slate-900/50 backdrop-blur-2xl border-white dark:border-slate-800 shadow-xl rounded-3xl">
            <CardHeader className="pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between">
                ...
                <Badge className="bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400 border-0 font-bold px-3 py-1 text-sm">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-[500px] overflow-y-auto custom-scrollbar">
                {localities.map((loc) => (
                  <div key={loc.id} className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors gap-4">
                    <div className="flex items-center gap-4">
                      <div className={`p-2.5 rounded-xl ${loc.isActive ? 'bg-emerald-100 dark:bg-emerald-900/50' : 'bg-slate-100 dark:bg-slate-800'}`}>
                        <MapPin className={`h-5 w-5 ${loc.isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`} />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto justify-between">
                      {loc.isActive ? (
                        <Badge className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-0 flex items-center gap-1.5 py-1 px-2.5 font-bold">
                          <CheckCircle2 className="h-3.5 w-3.5" /> Active
                        </Badge>
                      ) : (
                        <Badge className="bg-slate-100 dark:bg-slate-800 text-slate-500 border-0 flex items-center gap-1.5 py-1 px-2.5 font-bold">
                      ...
                      <Button 
                        ...
                        className={`font-bold rounded-xl ${!loc.isActive ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}
                      >
