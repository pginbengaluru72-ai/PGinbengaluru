  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        <Card className="col-span-1 border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-sm">
          <CardHeader>
            ...
            <form onSubmit={handleSubmitTicket} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <select 
                  ...
                  className="flex h-10 w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-xs font-bold"
                >
              ...
              <div className="space-y-1.5">
                ...
                <textarea 
                  ...
                  required
                  className="flex w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3 py-2 text-xs min-h-[100px] font-medium resize-none"
                  placeholder="e.g. AC remote is not working and fan speed is stuck..."
                />
              </div>

              <Button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl h-10 text-xs shadow-md">
        ...
        <Card className="col-span-1 md:col-span-2 border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-sm">
          <CardHeader>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {tickets.map((ticket) => (
                  <div key={ticket.id} className="grid grid-cols-5 p-3.5 items-center text-xs hover:bg-slate-50/50 dark:hover:bg-slate-950/50 transition-colors">
                    <div className="col-span-2 font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                    ...
                    <div>
                      <Badge variant="outline" className={
                        ...
                        ticket.status === 'In Progress' ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950 dark:text-amber-300 font-bold' :
                        ...
                        {ticket.status === 'In Progress' && <Clock className="mr-1 h-3 w-3 text-amber-500" />}
                        {ticket.status}
                      </Badge>
                    </div>
                    <div className="font-semibold text-slate-600 dark:text-slate-400">{ticket.priority}</div>
