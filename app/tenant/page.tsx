...
export default function TenantDashboardOverview() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-12">
        ...
        <motion.div whileHover={{ scale: 1.01 }} className={`p-4 border rounded-2xl flex items-start gap-3 shadow-sm transition-all ${broadcast.level === 'warning' ? 'bg-amber-50 border-amber-200 dark:bg-amber-950/40 dark:border-amber-800' : 'bg-indigo-50 border-indigo-200 dark:bg-indigo-950/40 dark:border-indigo-800'}`}>
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className={`text-xs font-bold uppercase tracking-wider ${broadcast.level === 'warning' ? 'text-amber-700 dark:text-amber-300' : 'text-indigo-700 dark:text-indigo-300'}`}>StaySure Platform Notice</span>
            </div>
      ...
      {/* MY INVOICES SECTION */}
      <div className="pt-6">
        <h3 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100 mb-4 flex items-center gap-2">
          <CreditCard className="w-5 h-5 text-indigo-600" /> My Invoices
        </h3>
        
        <div className="grid gap-4">
          {bills.length === 0 ? (
            <Card className="border-slate-200 dark:border-slate-800 shadow-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-8 text-center">
              <p className="text-sm text-slate-500 font-medium">No bills generated by your owner yet.</p>
            </Card>
          ) : (
            bills.map(bill => (
              <Card key={bill.id} className="border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900/90 hover:shadow-md transition-shadow">
                <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-slate-100 text-base">{bill.description}</h4>
                      <p className="text-xs font-medium text-slate-500 mt-0.5">
                        {bill.propertyName} • ID: {bill.publicId}
                      </p>
                      <p className="text-[11px] text-slate-400 mt-1">Generated: {new Date(bill.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end gap-6 sm:w-auto w-full border-t sm:border-0 pt-3 sm:pt-0 border-slate-100 dark:border-slate-800">
                    <div className="text-left sm:text-right">
                      <p className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
                        ₹{bill.amount.toLocaleString('en-IN')}
                      </p>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase inline-block mt-1 ${bill.status === 'PAID' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                        {bill.status}
                      </span>
                    </div>
                    {bill.status === 'PENDING' && (
                      <Button onClick={() => window.open(`https://wa.me/${bill.ownerPhone?.replace(/\\D/g, '') || ''}?text=${encodeURIComponent('Hello, I would like to pay my StaySure bill ('+bill.publicId+') for ₹'+bill.amount+'. Please share your UPI ID.')}`, '_blank')} className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-sm text-xs font-bold">
                        Pay via WhatsApp
                      </Button>
                    )}
                  </div>
